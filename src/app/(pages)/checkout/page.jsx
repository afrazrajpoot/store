...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    const savedOrderDetails = JSON.parse(localStorage.getItem('orderDetails') || 'null');
  ...
  const handlePlaceOrder = () => {
    const requiredFields = ['name', 'email', 'phone', 'address', 'country', 'city', 'region', 'postalCode'];
    const missingFields = requiredFields.filter(field => !customerDetails[field]);

    if (missingFields.length > 0) {
      toast.success(`Please fill in the following fields: ${missingFields.join(', ')}`,{
        duration: 3000,
        position: 'top-right',
  ...
  const customerFields = [
    ...
    { name: 'country', type: 'select', placeholder: 'Select Country', options: COUNTRIES },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <h2 className="text-2xl font-semibold mb-8">Customer Information</h2>
