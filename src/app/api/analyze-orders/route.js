  ...
  try {
    ...
    const monthlyData = Array(12).fill(0).map((_, index) => {
      const monthOrders = orders.filter((order) => {
        ...
        return orderDate.month() === index && orderDate.year() === moment().year();
      });
      ...
      return {
        ...
        pending: monthOrders.filter(order => order.status === "Pending").length
      };
    });
    ...
    const monthlyGrowth = monthlyData.map((month, index) => {
      ...
      const growth = previousMonth === 0 ? 0 : ((currentMonth - previousMonth) / previousMonth) * 100;
