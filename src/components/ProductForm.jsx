...
const ProductForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Failed to add product: " + (err.data?.message || err.message || 'Unknown error occurred'));
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <form onSubmit={handleSubmit} className="max-w-5xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="p-8">
          ...
          {/* Color Management */}
          <div className="mt-12">
            <h3 className="text-xl font-semibold text-gray-800 mb-6">Color Management</h3>
            <div className="space-y-4">
          ...
          {isError && error && (
            <div className="mt-4 p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg">
              {error?.data?.message || "An error occurred while adding the product"}
            </div>
          )}
        </div>
      </form>
